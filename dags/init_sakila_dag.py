"""
DAG: init_sakila_db
Description: Automates Sakila database setup with in-memory SQL syntax adaptation and COPY parsing for Postgres.
"""
...
@dag(
    ...
    description='Automated initialization of Sakila database (Schema + Data)',
    schedule='@once',
    start_date=datetime(2026, 1, 1),
    catchup=False,
    tags=['setup', 'sakila', 'postgres'],
)
def init_sakila_db():
    """
    ...
    create_schema = execute_sql_in_memory.override(task_id='create_sakila_schema')(
        file_path='/opt/airflow/sql/sakila/schema.sql',
        conn_id='postgres_sakila_conn',
        is_schema=True
    )

    insert_data = execute_sql_in_memory.override(task_id='insert_sakila_data')(
        file_path='/opt/airflow/sql/sakila/data.sql',
        conn_id='postgres_sakila_conn',
        is_schema=False
    )
